...
function AiretelPayment({value,user}) {
 ...
 */
 const onSubmitDepositHandler = async (event) => {
    ...
    const {PaymentSystem_id,displayName}=deposit["Airtel_Money"];
    depositHandler({
...
useEffect(()=>{
    if(user){
        setServiceDetails(user?.payment_services?.deposit["Airtel_Money"]);
    }
},[user]);
