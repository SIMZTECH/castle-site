...
function ZamtelPayment({value,user}) {
 ...
 */
 const onSubmitDepositHandler = async (event) => {
    ...
    const {user_id,currency,payment_services:{deposit},username}=user;
    const {PaymentSystem_id,displayName}=deposit["ZamtelInstant"];
    depositHandler({
...
useEffect(()=>{
    if(user){
        setServiceDetails(user?.payment_services?.deposit["ZamtelInstant"]);
    }
},[user]);
        ...
        <div className="flex-1 px-2 pt-2 pb-4 sm:max-w-[500px] mx-auto">
            ...
            {!isNil(serviceDetails?.infotext) && (
                <div className="px-2 text-gray-900 font-poppins">
                    ...
                    <div>{serviceDetails?.infotext}</div>
                </div>
            )}
        </div>
    )}
